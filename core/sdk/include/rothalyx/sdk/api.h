...
#define ROTHALYX_SDK_VERSION_MAJOR 1
#define ROTHALYX_SDK_VERSION_MINOR 0
#define ROTHALYX_SDK_VERSION_PATCH 3
#define ROTHALYX_SDK_ABI_VERSION 1
#define ROTHALYX_SDK_PLUGIN_API_VERSION "1"

typedef enum rothalyx_sdk_status_t {
    ROTHALYX_SDK_STATUS_OK = 0,
    ROTHALYX_SDK_STATUS_INVALID_ARGUMENT = 1,
    ROTHALYX_SDK_STATUS_NOT_FOUND = 2,
    ROTHALYX_SDK_STATUS_UNSUPPORTED = 3,
    ROTHALYX_SDK_STATUS_ERROR = 4
} rothalyx_sdk_status_t;

typedef struct rothalyx_project rothalyx_project_t;

typedef struct rothalyx_ai_options_t {
    const char* backend;
    const char* api_key;
    const char* model;
    const char* base_url;
    ...
    long timeout_ms;
} rothalyx_ai_options_t;

typedef struct rothalyx_run_overview_t {
    int run_id;
...
} rothalyx_run_overview_t;

typedef struct rothalyx_function_record_t {
    const char* name;
...
} rothalyx_function_record_t;

typedef struct rothalyx_ai_insight_record_t {
...
const char* rothalyx_sdk_status_string(rothalyx_sdk_status_t status);
