...
int main(int argc, char** argv) {
    ...
    const auto raw_path = std::filesystem::temp_directory_path() / "rothalyx_safe_parsing_raw.bin";
    {
    ...
    rothalyx::loader::LoadOptions options;
    options.policy.max_path_length = 4;
    if (rothalyx::loader::BinaryImage::load_from_file(raw_path, image, error, options)) {
    ...
    if (rothalyx::loader::BinaryImage::load_from_file(fixture_binary, image, error, options)) {
