#include <algorithm>
#include <array>
#include <cstddef>
#include <cstdint>
#include <cstring>
#include <filesystem>
#include <fstream>
#include <iostream>
#include <stdexcept>
#include <string_view>
#include <vector>

#include "rothalyx/disasm/disassembler.hpp"
#include "rothalyx/loader/binary_image.hpp"
#include "rothalyx/memory/address_space.hpp"

namespace {

template <typename T>
void write_value(std::vector<std::byte>& bytes, const std::size_t offset, const T value) {
    if (offset + sizeof(T) > bytes.size()) {
        throw std::runtime_error("test image write overflow");
    }
    std::memcpy(bytes.data() + offset, &value, sizeof(T));
}

void write_ascii(std::vector<std::byte>& bytes, const std::size_t offset, const std::string_view text) {
    if (offset + text.size() > bytes.size()) {
        throw std::runtime_error("test image write overflow");
    }
    std::memcpy(bytes.data() + offset, text.data(), text.size());
}

std::filesystem::path write_synthetic_macho() {
    constexpr std::size_t kImageSize = 0x550;
    constexpr std::size_t kHeaderOffset = 0x00;
    constexpr std::size_t kCommandsOffset = 0x20;
    constexpr std::size_t kTextSegmentOffset = kCommandsOffset;
    constexpr std::size_t kDataSegmentOffset = kTextSegmentOffset + 0x98;
    constexpr std::size_t kMainCommandOffset = kDataSegmentOffset + 0x98;
    constexpr std::size_t kSymtabOffset = kMainCommandOffset + 0x18;
    constexpr std::size_t kDysymtabOffset = kSymtabOffset + 0x18;
    constexpr std::size_t kDylibOffset = kDysymtabOffset + 0x50;
    constexpr std::size_t kCodeOffset = 0x200;
    constexpr std::size_t kImportPointerOffset = 0x300;
    constexpr std::size_t kSymbolTableOffset = 0x400;
    constexpr std::size_t kIndirectTableOffset = 0x480;
    constexpr std::size_t kRelocationTableOffset = 0x490;
    constexpr std::size_t kStringTableOffset = 0x500;

    std::vector<std::byte> bytes(kImageSize, std::byte{0});

    const std::array<std::uint8_t, 8> code_bytes{
        0x20, 0x00, 0x80, 0xD2,  // mov x0, #1
        0xC0, 0x03, 0x5F, 0xD6,  // ret
    };

    const std::string dylib_name = "libSystem.B.dylib";
    const std::string strings = std::string("\0_main\0_puts\0", 13);

    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x00, 0xFEEDFACF);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x04, 0x0100000C);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x08, 0);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x0C, 2);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x10, 6);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x14, 0x1E0);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x18, 0);
    write_value<std::uint32_t>(bytes, kHeaderOffset + 0x1C, 0);

    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x00, 0x19);
    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x04, 0x98);
    write_ascii(bytes, kTextSegmentOffset + 0x08, "__TEXT");
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x18, 0x100000000ULL);
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x20, 0x1000);
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x28, 0x0);
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x30, 0x208);
    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x38, 7);
    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x3C, 5);
    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x40, 1);

    write_ascii(bytes, kTextSegmentOffset + 0x48, "__text");
    write_ascii(bytes, kTextSegmentOffset + 0x58, "__TEXT");
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x68, 0x100000200ULL);
    write_value<std::uint64_t>(bytes, kTextSegmentOffset + 0x70, static_cast<std::uint64_t>(code_bytes.size()));
    write_value<std::uint32_t>(bytes, kTextSegmentOffset + 0x78, kCodeOffset);

    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x00, 0x19);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x04, 0x98);
    write_ascii(bytes, kDataSegmentOffset + 0x08, "__DATA_CONST");
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x18, 0x100001000ULL);
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x20, 0x1000);
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x28, kImportPointerOffset);
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x30, 0x10);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x38, 3);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x3C, 3);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x40, 1);

    write_ascii(bytes, kDataSegmentOffset + 0x48, "__la_symbol_ptr");
    write_ascii(bytes, kDataSegmentOffset + 0x58, "__DATA_CONST");
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x68, 0x100001000ULL);
    write_value<std::uint64_t>(bytes, kDataSegmentOffset + 0x70, 0x10);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x78, kImportPointerOffset);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x80, kRelocationTableOffset);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x84, 1);
    write_value<std::uint32_t>(bytes, kDataSegmentOffset + 0x88, 0x7);

    write_value<std::uint32_t>(bytes, kMainCommandOffset + 0x00, 0x80000028);
    write_value<std::uint32_t>(bytes, kMainCommandOffset + 0x04, 0x18);
    write_value<std::uint64_t>(bytes, kMainCommandOffset + 0x08, kCodeOffset);

    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x00, 0x2);
    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x04, 0x18);
    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x08, kSymbolTableOffset);
    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x0C, 2);
    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x10, kStringTableOffset);
    write_value<std::uint32_t>(bytes, kSymtabOffset + 0x14, strings.size());

    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x00, 0xB);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x04, 0x50);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x10, 0);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x14, 1);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x18, 1);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x1C, 1);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x38, kIndirectTableOffset);
    write_value<std::uint32_t>(bytes, kDysymtabOffset + 0x3C, 1);

    write_value<std::uint32_t>(bytes, kDylibOffset + 0x00, 0xC);
    write_value<std::uint32_t>(bytes, kDylibOffset + 0x04, 0x30);
    write_value<std::uint32_t>(bytes, kDylibOffset + 0x08, 0x18);
    write_ascii(bytes, kDylibOffset + 0x18, dylib_name + std::string(1, '\0'));

    std::memcpy(bytes.data() + kCodeOffset, code_bytes.data(), code_bytes.size());
    write_value<std::uint64_t>(bytes, kImportPointerOffset + 0x08, 0x100000200ULL);

    write_value<std::uint32_t>(bytes, kSymbolTableOffset + 0x00, 1);
    write_value<std::uint8_t>(bytes, kSymbolTableOffset + 0x04, 0x0F);
    write_value<std::uint8_t>(bytes, kSymbolTableOffset + 0x05, 1);
    write_value<std::uint16_t>(bytes, kSymbolTableOffset + 0x06, 0);
    write_value<std::uint64_t>(bytes, kSymbolTableOffset + 0x08, 0x100000200ULL);

    write_value<std::uint32_t>(bytes, kSymbolTableOffset + 0x10, 7);
    write_value<std::uint8_t>(bytes, kSymbolTableOffset + 0x14, 0x01);
    write_value<std::uint8_t>(bytes, kSymbolTableOffset + 0x15, 0);
    write_value<std::uint16_t>(bytes, kSymbolTableOffset + 0x16, 0x0100);
    write_value<std::uint64_t>(bytes, kSymbolTableOffset + 0x18, 0);

    write_value<std::uint32_t>(bytes, kIndirectTableOffset + 0x00, 1);
    write_value<std::uint32_t>(bytes, kRelocationTableOffset + 0x00, 0x8);
    write_value<std::uint32_t>(bytes, kRelocationTableOffset + 0x04, 0x06000000);
    write_ascii(bytes, kStringTableOffset, strings);

    const std::filesystem::path output_path =
        std::filesystem::temp_directory_path() / "rothalyx_synthetic_arm64.macho";
    std::ofstream output(output_path, std::ios::binary | std::ios::trunc);
    output.write(reinterpret_cast<const char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
    output.close();
    return output_path;
}

}  // namespace

int main() {
    const std::filesystem::path binary_path = write_synthetic_macho();

    rothalyx::loader::BinaryImage image;
    std::string error;
    if (!rothalyx::loader::BinaryImage::load_from_file(
            binary_path,
            image,
            error,
            rothalyx::loader::LoadOptions{.base_address = 0x1000, .rebase_address = 0x200000000ULL}
        )) {
        std::cerr << "load failed: " << error << '\n';
        return 1;
    }

    if (image.format() != rothalyx::loader::BinaryFormat::MachO) {
        std::cerr << "expected Mach-O format\n";
        return 2;
    }
    if (image.architecture() != rothalyx::loader::Architecture::ARM64) {
        std::cerr << "expected arm64 architecture\n";
        return 3;
    }
    if (!image.preferred_load_address().has_value() || *image.preferred_load_address() != 0x100000000ULL) {
        std::cerr << "unexpected preferred load address\n";
        return 4;
    }
    if (image.base_address() != 0x200000000ULL || image.rebase_delta() != 0x100000000LL) {
        std::cerr << "unexpected rebase result\n";
        return 5;
    }
    if (!image.entry_point().has_value() || *image.entry_point() != 0x200000200ULL) {
        std::cerr << "unexpected entry point after rebase\n";
        return 6;
    }
    if (image.sections().size() != 2) {
        std::cerr << "expected two mapped sections\n";
        return 7;
    }

    const auto imported = std::find_if(
        image.imports().begin(),
        image.imports().end(),
        [](const rothalyx::loader::ImportedSymbol& symbol) {
            return symbol.library == "libSystem.B.dylib" &&
                   symbol.name == "puts" &&
                   symbol.address == 0x200001000ULL;
        }
    );
    if (imported == image.imports().end()) {
        std::cerr << "expected arm64 puts import\n";
        return 8;
    }

    const auto exported = std::find_if(
        image.exports().begin(),
        image.exports().end(),
        [](const rothalyx::loader::ExportedSymbol& symbol) {
            return symbol.name == "main" && symbol.address == 0x200000200ULL;
        }
    );
    if (exported == image.exports().end()) {
        std::cerr << "expected main export\n";
        return 9;
    }

    const auto data_section = std::find_if(
        image.sections().begin(),
        image.sections().end(),
        [](const rothalyx::loader::Section& section) {
            return section.name == "__DATA_CONST:__la_symbol_ptr";
        }
    );
    if (data_section == image.sections().end() || data_section->bytes.size() < 16) {
        std::cerr << "expected mapped Mach-O data section\n";
        return 10;
    }

    std::uint64_t rebased_pointer = 0;
    std::memcpy(&rebased_pointer, data_section->bytes.data() + 8, sizeof(rebased_pointer));
    if (rebased_pointer != 0x200000200ULL) {
        std::cerr << "expected rebased Mach-O local pointer\n";
        return 11;
    }

    rothalyx::memory::AddressSpace address_space;
    if (!address_space.map_image(image)) {
        std::cerr << "failed to map Mach-O image\n";
        return 12;
    }

    rothalyx::disasm::Disassembler disassembler;
    if (!disassembler.is_supported(image.architecture())) {
        std::cerr << "expected ARM64 decode support\n";
        return 13;
    }

    const auto instructions = disassembler.decode(address_space, *image.entry_point(), 8, image.architecture());
    if (instructions.size() < 2 || instructions.front().mnemonic == "db") {
        std::cerr << "expected real ARM64 instructions\n";
        return 14;
    }

    return 0;
}
